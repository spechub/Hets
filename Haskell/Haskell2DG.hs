...
anaHaskellFileAux :: String -> DGraph -> IO (Maybe (LIB_NAME, HsModule, DGraph, LibEnv))
...
-- input: (so far) generated DGraph, a module's abstract syntax and its ModuleInfo
-- task: adds a new node (representing the module) to the DGraph
--       checks whether there are imported modules or not
createDGraph :: DGraph -> LIB_NAME -> AHsModule -> ModuleInfo -> IO(DGraph, LibEnv)
createDGraph dg ln (AHsModule name exps imps decls) modInfo = 
      -- create the node, representing the module
       let node_contents  | imps == [] =       -- module with no imports
                             DGNode {
        ...
        in
           case imps of
            -- no imports -> no other nodes and edges
             [] -> return (dg', (addDG2LibEnv emptyLibEnv ln node dg'))
            -- imports -> add imported Modules
             _  -> insImports ln dg' node emptyLibEnv imps

-- input: LibName of (current treated) 'main module', (so far generated) DGraph, 
--        node representing the 'main module', (so far generated) LibEnv, 
--        List of importDecls (from 'main module')
-- task: call anaHaskellFileAux for each imported module
--       add link between 'the' node and the node representing an imported module
insImports :: LIB_NAME -> DGraph -> Node -> LibEnv -> [AHsImportDecl] -> IO (DGraph,LibEnv)
insImports ln dg n le [] = return (dg, (addDG2LibEnv le ln n dg))
           ...
           case (Map.member lnTest le) of
               False -> do
                         -- analyse imported module just like the main module
                          Just(libName, _, dg', libEnv) <- anaHaskellFileAux (fileName name) dg
