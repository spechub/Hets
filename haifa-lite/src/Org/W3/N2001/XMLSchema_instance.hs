...
{-
xsiTypingOn = newDynamicKey "xsiTypingOn" True

-- | Type encoder hook, atm this is basically for adding XSD type attributes at appropriate nodes.
data XSITypeHook = XSITypeHook

instance XMLNamespace XSITypeHook where
    namespaceURI _ = Just thisNamespace


instance (XMLNamespace a, XSDType a) => XMLHook XSITypeHook a where
    encodeHook dm _ x = let ty = (xsdType x); on = lookupDM_D xsiTypingOn dm in 
			    if (null ty || not on) 
                                then addAttribute (repeat none)
                                else addAttribute $ map (\t -> attr (applyPrefix dm XSITypeHook "type") $ txt (applyPrefix dm x t)) ty
    
instance InitXMLHook XSITypeHook where
    hookDM _ = addNamespaces [(thisPrefix, thisNamespace)]

xsiTypeOff = \x -> x{setFlags = addToDM False xsiTypingOn (setFlags x)}
xsiTypeOffE = [| xsiTypeOff |]

xsiTypeOn = \x -> x{setFlags = addToDM True xsiTypingOn (setFlags x)}
xsiTypeOnE = [| xsiTypeOn |]
-}