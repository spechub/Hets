...
getVarSorts oldS sen = 
 case sen of
  CBasic.Quantification _ vdecls f _ -> 
     Set.union 
       (Set.fromList $ 
         map (\(CBasic.Var_decl _ s _) -> s) vdecls) $ getVarSorts oldS f
  CBasic.Junction _ fs _ -> foldl (\s f -> getVarSorts s f) oldS fs
  CBasic.Relation f1 _ f2 _ -> 
     Set.union (getVarSorts oldS f1) (getVarSorts oldS f2)
...
addSortToOpType stStr (CBasic.Op_type k w s r1) = 
  CBasic.Op_type k (stStr:w) s r1

addSortToPredType :: Id -> CBasic.PRED_TYPE -> CBasic.PRED_TYPE
addSortToPredType stStr (CBasic.Pred_type w r1) = 
  CBasic.Pred_type (stStr : w) r1
...
addVarToTerm stStr v t = case t of
 ...
 CBasic.Application (CBasic.Qual_op_name f o r2) args r -> 
   CBasic.Application 
    (CBasic.Qual_op_name f (addSortToOpType stStr o) r2) 
    ((CBasic.Qual_var v stStr nullRange):(map (addVarToTerm stStr v) args)) r
...
addX stStr v sen = case sen of
  CBasic.Quantification q vdecls f r -> 
    CBasic.Quantification q vdecls (addX stStr v f) r
  CBasic.Junction j fs r -> 
    CBasic.Junction j (map (addX stStr v) fs) r
  CBasic.Relation f1 rel f2 r -> 
    CBasic.Relation (addX stStr v f1) rel (addX stStr v f2) r
  CBasic.Negation f r -> 
    CBasic.Negation (addX stStr v f) r
  CBasic.Atom _ _ -> sen
  CBasic.Definedness t r -> 
    CBasic.Definedness (addVarToTerm stStr v t) r
  CBasic.Predication (CBasic.Qual_pred_name p t r2) terms r -> 
    CBasic.Predication 
      (CBasic.Qual_pred_name p (addSortToPredType stStr t) r2) 
      ((CBasic.Qual_var v stStr nullRange):(map (addVarToTerm stStr v) terms)) 
      r
  CBasic.Equation t1 eq t2 r -> 
      CBasic.Equation (addVarToTerm stStr v t1) eq (addVarToTerm stStr v t2) r
  _ -> error $ "Illegal argument for addX in HPAR2CASL.hs: " ++  show sen

replaceVarAppls :: CBasic.VAR -> CBasic.SORT -> CBasic.CASLFORMULA -> 
                   CBasic.CASLFORMULA
replaceVarAppls v s sen = case sen of
  CBasic.Quantification q vdecls f r -> 
    CBasic.Quantification q vdecls (replaceVarAppls v s f) r
  CBasic.Junction j fs r -> 
    CBasic.Junction j (map (replaceVarAppls v s) fs) r
  CBasic.Relation f1 rel f2 r -> 
    CBasic.Relation (replaceVarAppls v s f1) rel (replaceVarAppls v s f2) r
  CBasic.Negation f r -> 
    CBasic.Negation (replaceVarAppls v s f) r
  CBasic.Atom _ _ -> sen
  CBasic.Definedness t r -> 
    CBasic.Definedness (replTermWithVar v s t) r
  CBasic.Predication p terms r -> 
    CBasic.Predication p (map (replTermWithVar v s) terms) r
  CBasic.Equation t1 eq t2 r -> 
    CBasic.Equation (replTermWithVar v s t1) eq (replTermWithVar v s t2) r
  _ -> error $ 
        "Illegal argument for replaceVarAppls in HPAR2CASL.hs: " ++  show sen

replTermWithVar :: CBasic.VAR -> CBasic.SORT -> CBasic.TERM () -> 
                   CBasic.TERM ()
...
mapTheoryConstr :: (Comorphism cid lid sublogics basic_spec sen
                  ...
                  CSign.Symbol CMor.RawSymbol ProofTree,
                   ...
                   => 
                      ...
                      -- base comorphism
                   -> hlid   
                      -- hybrid logic, source of the generated comorphism. 
                      -- we only need it to 
...
mapTheoryConstr cid hlid (hsig, nhsens) = do
 ...
 let st = genName $ "ST_" ++ show hlid
  ...
  -- 3. variables in \Gamma_\Sigma
     cvars = foldl (\vars asen -> getVarSorts vars $ sentence asen) 
                   Set.empty csens 
  -- 4. e -> forall gn_X : gn_ST . e(X) 
     csens' = -- trace ("cvars:" ++ show cvars) $ 
              map (\f -> f { sentence = CBasic.Quantification 
                                          CBasic.Universal 
                                          [CBasic.Var_decl [v] st nullRange]
                                          (addX st v $ sentence f) nullRange} )
                  csens
  -- 5. this one has gn_ST
 stsig <- CSign.addSymbToSign (CSign.emptySign ()) $
           CSign.Symbol st CSign.SortAsItemType
  -- 6. this one has [S_\Sigma]
 sortsig <- foldM (\aSig x -> CSign.addSymbToSign aSig $ 
                                CSign.Symbol x CSign.SortAsItemType) 
                   stsig $ Set.toList $ CSign.sortSet csig
 -- 7. this one has \overline{Nom}
 nomsig <- foldM (\aSig x -> CSign.addSymbToSign aSig $ 
                               CSign.Symbol x $ CSign.OpAsItemType $ 
                                 CSign.OpType CBasic.Total [] st) 
                  sortsig $ GTypes.noms hsig
  -- 8. this one has [F_\Sigma]
 opsig <- foldM (\aSig (i, CSign.OpType k w s) -> 
                   CSign.addSymbToSign aSig $ 
                      CSign.Symbol i $ CSign.OpAsItemType $ 
                                   CSign.OpType k (st:w) s)
               ...
               let ydecl = CBasic.mkVarDecl (genToken "w") st
                   ...
                   df = CBasic.mkForall [ydecl] $
                          CBasic.mkForall xsdecl $
                           CBasic.mkImpl 
                             (CBasic.conjunct $ 
                                map (\xidecl -> case xidecl of 
                                       CBasic.Var_decl [xi] si _ -> 
                                        CBasic.mkPredication 
                                          (CBasic.mkQualPred domain $ 
                                           CBasic.Pred_type [st, si] nullRange)
                                          [CBasic.Qual_var (genToken "w") 
                                                           st nullRange,
                             ...
                             (CBasic.mkPredication 
                               (CBasic.mkQualPred domain $ 
                                 CBasic.Pred_type [st, s] nullRange) 
                                [CBasic.Qual_var (genToken "w") st nullRange,
                                 CBasic.mkAppl 
                                  (CBasic.Qual_op_name f 
                                    (addSortToOpType st $ CSign.toOP_TYPE o) 
                                    nullRange) 
 ...
 predsig <- foldM (\aSig (i, CSign.PredType w) -> 
                     CSign.addSymbToSign aSig $ 
                       CSign.Symbol i $ CSign.PredAsItemType $ 
                                      CSign.PredType (st:w))
                   opsig $ MapSet.toPairList $ CSign.predMap csig
  -- 11. this one has D_s
 domsig <- foldM (\aSig s -> 
                  CSign.addSymbToSign aSig $ 
                   CSign.Symbol domain $ CSign.PredAsItemType $ 
                                         CSign.PredType [st, s])
            predsig $ Set.toList $ CSign.sortSet csig
  -- 12. this one has \overline{\Lambda}
 lamsig <- foldM (\aSig (l, x) -> 
                    CSign.addSymbToSign aSig $ 
                      CSign.Symbol l $ CSign.PredAsItemType $ 
                                       CSign.PredType $ take x $ repeat st)
            domsig $ Map.toList $ GTypes.mods hsig 
 -- 13. translate sentences from HPAR to CASL
 let ncsens = map (mapNamedSentence (show hlid) mapBTheory 
                                    mapBSen baseLid hsig) 
                  nhsens
...
mapNamedSentence :: Logic lid sublogics basic_spec sen
                  ...
                  mor sym raw_sym proof_tree =>
                    ...
                    GTypes.HSign sig -> 
                    Named (GTypes.HFORMULA sen symb_items raw_sym) -> 
                    Named CBasic.CASLFORMULA
mapNamedSentence hl mapBTheory mapBSen baseLid hsig nsen = 
  nsen {sentence = mapHSentence hl mapBTheory mapBSen baseLid hsig $ 
                   sentence nsen}
...
isVar nom hth = 
 let 
  nomVars = concatMap (\x -> case x of 
                               GTypes.ASymbol (GTypes.HSymb n GTypes.Nom) ->
                                    [n]
                               _ -> []) $ 
               GMethods.hVars hth
...
mapSentenceAux hl mapBTheory mapBSen baseLid 
               hth x st index hsen = case hsen of
 GTypes.Nominal _s b i _ -> CBasic.mkStEq 
                          (CBasic.Qual_var x st nullRange) $ 
                          if b then CBasic.Qual_var i st nullRange
                          else CBasic.Application 
                                 (CBasic.mkQualOp (simpleIdToId i) $ 
                                  CBasic.Op_type CBasic.Total [] st nullRange)
                                 [] nullRange
 ...
 GTypes.AtState _s nom hf _ -> 
   let cf = mapSentenceAux hl mapBTheory mapBSen baseLid hth x st index hf
       t = if isVar nom hth then CBasic.Qual_var nom st nullRange else 
              CBasic.mkAppl (CBasic.mkQualOp (simpleIdToId nom) $ 
                             CBasic.Op_type CBasic.Total [] st nullRange) 
                            []
   in CInd.substitute x st t cf 
 -- mapSentenceAux i st sen does not work, 
 -- because i should not be a var. apply a substitution of x with i instead
 GTypes.Negation hf _ -> 
   CBasic.Negation 
    (mapSentenceAux hl mapBTheory mapBSen baseLid 
                    hth x st index hf) nullRange
 GTypes.Conjunction hfs _ -> 
   CBasic.Junction CBasic.Con 
                   (map (mapSentenceAux hl mapBTheory mapBSen baseLid 
                                        hth x st index) hfs) nullRange
 GTypes.Disjunction hfs _ -> 
   CBasic.Junction CBasic.Dis 
                   (map (mapSentenceAux hl mapBTheory mapBSen baseLid 
                                        hth x st index) hfs) nullRange
 GTypes.Implication hf1 hf2 _ -> 
   CBasic.Relation (mapSentenceAux hl mapBTheory mapBSen baseLid 
                                      hth x st index hf1) 
                   CBasic.Implication 
                    (mapSentenceAux hl mapBTheory mapBSen baseLid 
                                       hth x st index hf2) 
                    nullRange
 GTypes.Equivalence hf1 hf2 _ -> 
   CBasic.Relation (mapSentenceAux hl mapBTheory mapBSen baseLid 
                                      hth x st index hf1) 
                   CBasic.Equivalence 
                    (mapSentenceAux hl mapBTheory mapBSen baseLid 
                                       hth x st index hf2) 
                   nullRange
 GTypes.BoxFormula _s md hf _ -> 
   let yName = genToken $ "Y" ++ show index in
    CBasic.mkForall [CBasic.mkVarDecl yName st] $ 
     CBasic.mkImpl 
      (CBasic.mkPredication (CBasic.mkQualPred (simpleIdToId md) $ 
 ...
 GTypes.DiamondFormula _s md hf _ -> 
   let yName = genToken $ "Y" ++ show index in
    CBasic.mkExist [CBasic.mkVarDecl yName st] $ 
     ...
     [CBasic.mkPredication (CBasic.mkQualPred (simpleIdToId md) $ 
      ...
      mapSentenceAux hl mapBTheory mapBSen baseLid hth yName st (index+1) hf]
     nullRange
 GTypes.QuantNominals q noms hf _ -> 
   (case q of 
   ...
   (map (\n -> CBasic.mkVarDecl n st) noms)
    $ mapSentenceAux hl mapBTheory mapBSen baseLid
       (hth {
         GMethods.hVars = foldl (\l n -> (GTypes.ASymbol $ 
                                           GTypes.HSymb (simpleIdToId n) 
                                                        GTypes.Nom)
                                         :l) 
                          (GMethods.hVars hth) noms
            }
       ) 
       x st index hf 
 GTypes.QuantVarsParse _ _ _ _ -> 
   error "GenHyb.GenComor.mapSentenceAux: sentence not parsed"
 GTypes.QuantVars q vars hf _ -> let 
   hsig = GMethods.hSign hth
   domainSen xi si = CBasic.mkPredication 
                      (CBasic.mkQualPred (genName $ "domain_" ++ hl) $ 
                        CBasic.Pred_type [st, si] nullRange)
                      [CBasic.Qual_var x st nullRange,
                        CBasic.mkAppl (CBasic.Qual_op_name (simpleIdToId xi)
                                        (addSortToOpType st $ 
                                          CBasic.Op_type CBasic.Total 
                                                         [] si nullRange)
                                       nullRange) 
                        [CBasic.Qual_var x st nullRange]]
   ...
   getCSens vi = let
    ...
    in case resTh'' of
         Nothing -> 
          error $ 
           "could not translate theory with added variable:" ++ show ds''
         Just (_, sens) -> sens
   --  keep the sentences that appear only in the second list of sentences
   gamma_xi vi = map (\f -> 
                       f { sentence = CBasic.Quantification 
                                       CBasic.Universal 
                                       [CBasic.Var_decl [genToken "w"] 
                                                        st nullRange] 
                                       (addX st (genToken "w") $ sentence f)
                                       nullRange
                         }
                     ) $ 
                     filter (\s -> not (s `elem` csens)) $ 
                     getCSens vi
   ...
   cvars = map (\rs -> case raw_to_var baseLid rs of
                        Nothing -> error $ 
                                    "could not convert symbol to variable: " 
                                    ++ show rs
                        Just (v, s) -> (v, s) ) vars
   senWithVarAppl = let 
                      conds = CBasic.Junction CBasic.Con (
                                      (map sentence $ concatMap gamma_xi vis)
                                      ++ 
                                      (map (\(xi,si) -> domainSen xi si) 
                                           cvars)
                              )nullRange
                      mapHF = mapSentenceAux hl mapBTheory mapBSen baseLid 
                                             (hth {GMethods.hSign = hsig'}) 
                                             x st index hf
                    in
                      case q of 
                        GTypes.HUniversal _ -> 
                          CBasic.mkImpl conds mapHF 
                        GTypes.HExistential _ -> 
                          CBasic.conjunct [conds, mapHF]
   senRepl = foldl (\csen (xi,si) -> replaceVarAppls xi si csen) 
                   senWithVarAppl cvars
  in (case q of 
       GTypes.HUniversal _ -> CBasic.mkForall
       GTypes.HExistential _ -> CBasic.mkExist)
