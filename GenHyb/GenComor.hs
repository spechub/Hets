...
mapTheoryConstr cid hlid (hsig, nhsens) = do
 ...
 let ncsens = map (mapNamedSentence mapBTheory mapBSen baseLid hsig) nhsens
...
{-
mapHTheory :: Logic lid sublogics basic_spec sen
                  symb_items symb_map_items sig
                  mor sym raw_sym proof_tree =>
              ((sig, [Named sen]) -> Result (CSign.CASLSign, [Named CBasic.CASLFORMULA])) -- theory translation
           -> (sig -> sen -> Result CBasic.CASLFORMULA) -- sentence translation
           -> lid
           -> [SemanticConstraint]
           -> (GTypes.HSign sig, [Named (GTypes.HFORMULA sen symb_items raw_sym)]) ->
              Result (CSign.CASLSign, [Named CBasic.CASLFORMULA])
mapHTheory mapBTheory mapBSen baseLid semConstr (hsig, nhsens) = do
 -- 1. map base signature
 (csig, csens) <- mapBTheory (GTypes.baseSig hsig, [])
 -- 2. auxiliaries
 let st = genName "ST"
 ...
 constrsens <- foldM (\l1 sc -> do 
                         l2 <- constr_to_sens hlid hsig sc
                         return $ l1 ++ l2) [] $ sem_constr hlid        
 -- constrsens = concatMap (constr_to_sens baseLid (GTypes.baseSig hsig)) semConstr -- TODO: these should be from hLid
 -- 15. this is V(\Gamma_Sigma)
 let makeVSen s = makeNamed ("ga_V_"++show s)
                   $ CBasic.mkForall [CBasic.mkVarDecl (genToken "w") st, CBasic.mkVarDecl (genToken "x") s] 
                   $ CBasic.mkPredication (CBasic.mkQualPred domain $ CBasic.Pred_type [st, s] nullRange)
...
mapNamedSentence mapBTheory mapBSen baseLid hsig nsen = nsen {sentence = mapHSentence mapBTheory mapBSen baseLid hsig $ sentence nsen}
...
mapHSentence mapBTheory mapBSen baseLid hsig hsen = 
 let 
  x = genToken "X"
  st = genName "ST"
  hth = GMethods.HTheoryAna hsig Set.empty [] [] emptyGlobalAnnos []
 in CBasic.mkForall [CBasic.mkVarDecl x st] $ 
       mapSentenceAux mapBTheory mapBSen baseLid hth x st hsen
...
mapSentenceAux mapBTheory mapBSen baseLid hth x st hsen = case hsen of
 ...
 GTypes.Base_formula f _ -> let
  ...
  in case f' of
      ...
      Just f'' -> addX x f''
 GTypes.AtState nom hf _ -> let cf = mapSentenceAux mapBTheory mapBSen baseLid hth x st hf 
 ...
 GTypes.Negation hf _ -> CBasic.Negation (mapSentenceAux mapBTheory mapBSen baseLid hth x st hf) nullRange
 GTypes.Conjunction hfs _ -> CBasic.Junction CBasic.Con (map (mapSentenceAux mapBTheory mapBSen baseLid hth x st) hfs) nullRange
 GTypes.Disjunction hfs _ -> CBasic.Junction CBasic.Dis (map (mapSentenceAux mapBTheory mapBSen baseLid hth x st) hfs) nullRange
 GTypes.Implication hf1 hf2 _ -> CBasic.Relation (mapSentenceAux mapBTheory mapBSen baseLid hth x st hf1) 
                                                 CBasic.Implication 
                                                 (mapSentenceAux mapBTheory mapBSen baseLid hth x st hf2) 
                                                 nullRange
 GTypes.Equivalence hf1 hf2 _ -> CBasic.Relation (mapSentenceAux mapBTheory mapBSen baseLid hth x st hf1) 
                                                 CBasic.Equivalence 
                                                 (mapSentenceAux mapBTheory mapBSen baseLid hth x st hf2) 
                                                 nullRange
 GTypes.BoxFormula md hf _ -> CBasic.mkForall [CBasic.mkVarDecl (genToken "Y") st] $ 
                                               CBasic.mkImpl 
                                                 ...
                                                 mapSentenceAux mapBTheory mapBSen baseLid hth (genToken "Y") st hf
 ...
 GTypes.QuantNominals q noms hf _ -> (case q of 
                                      ...
                                      $ mapSentenceAux mapBTheory mapBSen baseLid
 ...
 GTypes.QuantVars q vars hf _ -> let 
   ...
   gamma_xi vi = map (\f -> f { sentence = CBasic.Quantification CBasic.Universal [CBasic.Var_decl [genToken "w"] st nullRange] (addX (genToken "w") $ sentence f) nullRange} )
   ...
   senWithVarAppl = CBasic.mkImpl 
                    ...
                    $ mapSentenceAux mapBTheory mapBSen baseLid (hth {GMethods.hSign = hsig'}) x st hf
