...
basic_Rel_analysis (spec, sign, _) =
    ...
    do
        ...
        mapM_ (analyse_relationship os) sens
        return (spec, ExtSign
                    {
                        plainSign = os
...
collectTypes tb = map (collectType tb)
...
" right hand side of: " ++ (show rel) ++ " do not match") rn) -}
            mapM_ (analyse_RSQualid rn tb) relDom
            k2 <- mapM (analyse_RSQualidK rn tb) relCo
            let kl2 = Set.fromList $ map fromJust $ filter (\ x -> case x of
                                 Nothing -> False
                                 _ -> True) $ map snd k2
            when (kl2 /= Set.map fst keyz2) (fatal_error
             ("Not all keys are used on the right hand side of: " ++ show rel)
             rn)
            return reli
