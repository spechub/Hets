...
printMorphism m = pretty (source m) <> text "-->" <> pretty (target m)
  ...
  <> pretty y <> rparen) $ Map.toList $ propMap m) -- TODO: might have to sort it!
...
-- | creates a Morphism
mkMorphism :: Sign.Sign -> Sign.Sign -> Map.HashMap Id Id -> Morphism
mkMorphism s t p =
  Morphism { source = s
           , target = t
           , propMap = p }
...
morphismUnion mor1 mor2 =
  let pmap1 = propMap mor1
      ...
      p1 = source mor1
      p2 = source mor2
      up1 = Set.difference (allItems p1) $ Set.fromList $ Map.keys pmap1
      up2 = Set.difference (allItems p2) $ Set.fromList $ Map.keys pmap2
