...
import Text.XML.HXT.Arrow.XmlFilterInterface	( canonicalizeAllNodes
						, collapseAllXText
						)
import Text.XML.HXT.Arrow.XmlStateFilterInterface ( propagateAndValidateNamespaces
						  , getDocumentContents
						  , parseXmlDocument
						  )
...
import Text.XML.HXT.RelaxNG.Utils	( qn2String
					, formatStringList
					, compareURI
					)
...
validateRelax xmlDoc
  = fromLA $ createPatternFromXmlTree
             >>>
...
validateXMLDoc al xmlDoc
  = validateRelax $< ( readForRelax al xmlDoc
                       >>>
		       normalizeForRelaxValidation
                       >>>
                       getChildren
...
textDeriv _ Text _ = Text  
textDeriv cx1 (Value (uri, s) value cx2) s1 =
  case datatypeEqual uri s value cx2 s1 cx1 of
    Nothing     -> Empty 
    Just errStr -> NotAllowed errStr

textDeriv cx (Data (uri, s) params) s1 =
  case datatypeAllows uri s params s1 cx of
    Nothing     -> Empty 
    Just errStr -> NotAllowed errStr
...
textDeriv cx (DataExcept (uri, s) params p) s1 =
  case (datatypeAllows uri s params s1 cx)  of
    Nothing     -> if not $ nullable $ textDeriv cx p s1 
                   then Empty 
                   else NotAllowed $ "Any value except '" ++ show p ++ 
                                     "' expected, but value '" ++ s1 ++ "' found"
    Just errStr -> NotAllowed errStr
...
textDeriv cx (List p) s
  = if nullable (listDeriv cx p (words s)) 
    then Empty
    else NotAllowed $ "List with value(s) " ++ show p ++ 
                      " expected, but value(s) " ++ 
                      formatStringList ", " (words s) ++ " found"
textDeriv _ n@(NotAllowed _) _ = n
textDeriv _ p s = 
...
listDeriv _ p [] = p
listDeriv cx p (x:xs) = listDeriv cx (textDeriv cx p x) xs
...
startTagOpenDeriv n@(NotAllowed _) _ = n
...
attsDeriv _ _ _  = NotAllowed "Call to attsDeriv with wrong arguments"
...
attDeriv _ n@(NotAllowed _) _ = n
attDeriv _ p att = NotAllowed $ "Attribute-Pattern for Attribute " ++  show att ++ 
...
childrenDeriv cx p [] = childrenDeriv cx p [(NTree (XText "") [])]
childrenDeriv cx p [(NTree (XText s) children)] =
  let p1 = childDeriv cx p (NTree (XText s) children)
