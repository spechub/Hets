...
module Text.XML.HXT.Parser.XmlParser
    ...
    , parseXmlContent
    , parseXmlText
    -- added by Klaus Lüttich for better usage in the context of hets
    , parseXmlFromString
    , parseXmlDoc
    , parseXmlPart
    , parseXmlDTDPart
...
parseXmlText p loc
    = parseXmlFromString p loc . xshow . this


-- |
-- the parser driver, called from 'parseXmlText' and 'xread'

parseXmlFromString	:: Parser XmlTrees -> String -> String -> XmlTrees
parseXmlFromString p loc str
    = case ( parse p loc str ) of
	Right xl
	    -> xl
        Left  e
	    -> xerr (show e ++ "\n")
