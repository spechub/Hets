...
inferenceParent = lexeme $
   (inferenceRule >>= return . PInferred) <|>
   do
    t' <- genTerm
...
axioms l p = case inference p of
              File _ _ | role p == Axiom -> (formulaName . inference $ p,name p):l
              _ -> l
...
digraph :: Set.Set String -> Map.Map String String -> (Set.Set String,Set.Set String,
           String,Map.Map (String,[String]) String)
           -> ProofStep -> (Set.Set String,Set.Set String,
                            String,Map.Map (String, [String]) String)
digraph p_set aliases (s,neg,d,m) p =
 let s' = facts s p
     ...
     alias' n = case Map.lookup n aliases of
                 Just n' -> n'
                 Nothing -> n
     neg' = case inference p of
             (InferenceRule _ st _ _) ->
              if elem (map toLower st) ["cth","unc","uns"]
              then if negated then neg  else neg_
              else if negated then neg_ else neg
     ...
     (s'',neg'',m',d') = (case role p of
           ...
           Other      ->
            let (s'_,neg'_,m'',d'') = case inference p of
                 ProofOf tm    ->
                  (s',neg',m,[edge True ("v"++alias' tm) ("v"++alias' (name p))
                      [("label",(if negated then ("dis") else "")++"proves")]])
                 r@(Rule _ _) -> (s',neg',m,
                                   [edge False ("v"++alias' (parent r)) ("v"++alias' (name p))
                                     [("label",rule r)]])
                 r@(NoRule _) -> (s',neg',m,
                                 [edge False ("v"++alias' (parent r)) ("v"++alias' (name p)) []])
                 inf@(InferenceRule _ _ _ _) ->
                  let (r,l1,m1,new) = case Map.lookup (rule inf,termParents $
                                                parents inf) m of
                                Just r' -> (r',
                                 [edge False r' ("v"++alias' (name p)) []],m,False)
                                _       ->
                                 let r1 = "r"++(show $ Map.size m)
                                 in (r1,[vertex r1
                                     [("label","Apply rule "++rule inf),
                                      ("style","dotted")],
                                     if not isFact' then edge False r1 ((if head (name p) == 'r' then "" else "v")++alias' (name p)) [] else ""],
                                      Map.insert (rule inf, termParents $
                                       parents inf) r1 m, True)
                      (s2',neg2',l3,m3) = foldl (\(s1',neg1',l2,m2) s1 -> case s1 of
                                 PTerm s2  -> if new then
                                  (s1',neg1',l2++[edge False ("v"++alias' s2) r [],
                                        edge (not $ Set.member s2 s')
                                             ("v"++alias' s2) r [("label","SZS " ++ status inf),("color","blue"),("fontcolor","blue")] ],m2)
                                  else (s1',neg1',l2,m2)
                                 PInferred ir -> let (s1'',neg1'',l2',m2') = digraph p_set aliases (s1',neg1',unlines l2,m2) $
                                                                              ProofStep r Other Nothing ir
                                                 in (s1'',neg1'',[l2'],m2')
                                 PBuiltinTheory s2 ->
                                  case Map.lookup (s2,[]) m2 of
                                   Just s2'' -> (s1',neg1',l2++[edge False s2'' r []],m2)
                                   Nothing  ->
                                    let s2'' = "t"++(show $ Map.size m2)
                                    in (s1',neg1',l2++[vertex s2'' $ [("style","filled"),
                                             ("fillcolor","green"),
                                             ("color","green"),("label",s2)],
                                             edge False s2'' r []],
                                       Map.insert (s2,[]) s2'' m2)) (s',neg',l1,m1)
                                (Set.toList $ parentsOf inf)
                  in (s2',neg2',m3,l3)
                 _            -> (s',neg',m,[])
            in (s'_,neg'_,m'',case formula p of
                               Just _ | not isFact' -> (vertex ("v"++name p) $ color++[("label",showTerm p)]):d''
                               _ -> d''))
 in if (Set.member (name p) p_set || formula p == Nothing) &&
       not (elem (name p) $ Map.keys aliases)
