...
getSymbolMappingResults signatureMorphismKey = do
  symbolData <-
    select $ from $ \(signature_morphisms `InnerJoin` symbol_mappings
      ...
      on (symbolLoc_id_basesTarget ^. LocIdBaseId ==. coerceId (symbolsTarget ^. SymbolId))
      on (symbolLoc_id_basesSource ^. LocIdBaseId ==. coerceId (symbolsSource ^. SymbolId))
      on (coerceId (symbolsTarget ^. SymbolId) ==. symbol_mappings ^. SymbolMappingTargetId)
      on (coerceId (symbolsSource ^. SymbolId) ==. symbol_mappings ^. SymbolMappingSourceId)
