...
Isabelle built-in type-checking. Hets \cite{MossaTh,HetsUG,Hets} is an
...
Haskell programs.  Programatica already includes a translation to Isabelle/HOLCF which,
...
Our translation is defined recursively.  It is based on a translation
of names for avoidance of name clashes that is not specified here. We
write $\alpha'$ for both the recursive translation of item $\alpha$
and the renaming according to the name translation.  The translation
of types is given by the following rules:
...
Here, we rely on a specific Isabelle theory \emph{HsHOLCF} included in
the Hets distrubution. It defines the lifted products and lifted
...
which it is member.  Built-in types are translated to the lifting of ths
corresponding HOL type.  The Isabelle/HOLCF type constructor \emph{lift} is used to
lift types to flat domains.  

The types of Haskell functions and product
are translated, respectively, to Isabelle/HOLCF function spaces and lazy product ---
i.e.  such that $\bot = (\bot * \bot) \neq (\bot*~'a) \neq ('a * \bot)$. Type
constructors are translated to corresponding Isabelle/HOLCF ones (notably, parameters
precede type constructors in Isabelle syntax). \emph{Maybe} is
translated to Isabelle/HOLCF-defined \emph{maybe} (the disjoint union of the lifted
