...
pdfauthor={Till Mossakowski, Christian Maeder, Klaus Lüttich},
...
(see Fig.~\ref{fig:lang} for a simple subset)
...
laid out in \cite{Habil}. A short overview is given in \cite{MossakowskiEA06,MossakowskiEtAl07b}.
...
\end{description}
Propositional, SoftFOL and \Isabelle are currently the only logics
coming with a prover. Proof support for the other logics can be
obtained by using logic translations to a prover-supported logic.
...
%CoPCFOL2CoCFOL & coding of partial functions by error elements,
%similar to PCFOL2CFOL \\\hline
...
%PCFOL2CFOL & coding of partial functions by error elements
%(translation $(4a')$ of \cite{Mossakowski02}) \\\hline
...
\begin{verbatim}
            ...
            | (sig|th)[.delta]
            | pp.(het|tex|html)
            | graph.(exp.dot|dot|ps|davinci)
            | ast.(het|trm|taf|html|xml)
            | (fdg|hdg|dg)[.nax].(het|trm|taf|html|xml)
            | dfg[.c]
            | tptp[.c]
\end{verbatim}
The default is \texttt{prf} (a synonym for \texttt{dg.taf}), which
means that the development graph of the library is stored in textual
ATerm format (\texttt{taf}).  This format can be read in when a
library is downloaded from another library, avoiding the need to
re-analyse the downloaded library.

The \texttt{env} format is currently not used.
...
\item[\texttt{-I}, \texttt{--interactive}] run \Hets in interactive mode (experimental)
\end{description}
...
provers (currently, these are \Isabelle, MathServe Broker, \SPASS, and
Vampire, described below). By pressing `Prove' the selected prover is
launched and the theory along with the selected goals is translated
via the shortest possible path of comorphisms into the provers logic.
The button `More fine grained selection...' lets you pick a (composed)
comorphism in a separate window from where the prover is launched
then.

Since the amount and kind of sentences send to an ATP system is a
major factor for the performance of the ATP system. It is possible to
select the axioms and proven theorems which will comprise the theory
...
prover for first-order logic with equality. Further it provides a soft
...
The output is stored in the so-called
                \index{ATerms}%
