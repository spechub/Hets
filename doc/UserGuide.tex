...
\end{figure}
\ednote{TODO Christian: update table}
...
\ednote{TODO Christian: describe the following options}
MISSING OPTIONS:
\begin{verbatim}
  -x                --xml                    use xml packets to communicate
  -c HOSTNAME:PORT  --connect=HOSTNAME:PORT  run interface comunicating via connecting to the port
  -S PORT           --listen=PORT            run interface by listening to the port
  -d STRING         --dump=STRING            dump various strings
  -A                --apply-automatic-rule   apply automatic dev-graph strategy
  -N                --normal-form            compute normal forms (takes long)
\end{verbatim}
...
re-analyse the downloaded library. You can also directly read in
the \texttt{prf} format (both from the command line, by calling
\Hets on the \texttt{prf} file, and from the GUI, using the
File-Open menu.
