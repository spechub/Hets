...
\begin{figure}
  \begin{center}
    \includegraphics[width=\textwidth]{LogicGraph}
  \end{center}
...
\item[OWL DL] is the description logic sublanguage of the Web Ontology Language (OWL \cite{McGvHa:owl04})\ednote{CL: So far this reads like OWL 1; how about OWL 2 support?} recommended by the
...
\item[SoftFOL] \cite{LuettichEA06a} offers three automated theorem
  ...
  \cite{WeidenbachEtAl02}; \item Vampire \cite{RiazanovV02}; and \item 
  MathServe Broker\footnote{which chooses an appropriate ATP upon a
    classification of the FOL problem} \cite{ZimmerAutexier06}\end{inparaenum}.
...
CLIF files:\ednote{CL: The previous revision of this document used ``cl-text'', which Hets also supports for some reason, but the Common Logic specification says ``cl:text''.}\\
