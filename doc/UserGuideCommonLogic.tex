...
\subsection{Common Logic Interchange Format (CLIF)}
...
CLIF syntax, and also recursively reads in any imported files.

CLIF itself does not support the specification of logical
consequences, nor relative theory interpretations, nor other
features that speak about structuring and comparing logical
theories. Michael Gr\"uninger has suggested some CLIF annotations for
this purpose, which are supported by \Hets, see
section~\ref{relationsInCL}. Alternatively, CLIF syntax can be used
within \HetCASL files, or CLIF files can be refered to within \HetCASL
files.  \HetCASL is a structuring languages supporting relative theory
interpretations and other things, see section~\ref{HetCASL} below.

\subsection{OWL2}

OWL2 is a W3C standard, see \url{http://www.w3.org/TR/owl2-overview/}.
\Hets can directly read in OWL2 files in various syntaxes, including
the native XML syntax, the RDF syntax, and the human-readable
Manchester syntax.
...
\HetCASL files, or OWL2 files can be refered to within \HetCASL files.
...
SPEC ::= BASIC-SPEC                %% logic-specific syntax, e.g CLIF, Manchester syntax
...
DEFINITION ::= logic ID            %% select a new logic for subsequent items
             ...
             | view ID : SPEC to SPEC = SYMBOL-MAP end  %% interpretation of theories
             | view ID : SPEC to SPEC = logic ID end    %% dto., but across different logics
...
Hets calls an external OWL2 parser
  ...
  imports. \begin{newpart}{CL: ``Full/DL/Lite'' is OWL 1 terminology.  I've updated it to OWL 2 profiles, but please check if I got it right.}The Java parser also does a first analysis classifying
 ...
 Hets supports all except OWL Full.\end{newpart}
The
...
\item Eprover \cite{Schulz:AICOM-2002}, see {http://www.eprover.org};
\item E-KRHyper \cite{DBLP:conf/cade/PelzerW07}, see {http://www.uni-koblenz.de/~bpelzer/ekrhyper}, and 
\item 
