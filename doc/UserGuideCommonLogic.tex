...
CLIF syntax, and also recursively reads in any imported files (cf.\ section~\ref{relationsInCL} for the syntax).
...
\Hets can directly read in OWL2 files in various syntaxes (called ``serializations''), including
the native OWL XML syntax \cite{w3c:owl2-xml}, the RDF/XML syntax \cite{w3c:owl2-RDF-mapping,w3c04:rdf-xml}\ednote{CL: This was previously ``the RDF syntax''; however RDF is just a data model, which has many possible syntaxes itself.  I presume that Hets supports at least RDF/XML, but does it also supports further RDF serializations, e.g.\ Turtle?}, and the human-readable
Manchester syntax \cite{w3c:owl2-manchester}.
