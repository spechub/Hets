...
\begin{quote}
\textbf{view} v : sp1 \textbf{to} sp2 \textbf{end}
\end{quote}
  Views are declared outside of specifications, as can be seen from listing~\ref{fig:lang}.
...
(name map) in a relation.\ednote{CL: Would it be worth mentioning that, if you want to achieve something comparable to ``importation with renaming'', you can use \HetCASL's ``SPEC with ... then SPEC'' extension mechanism?} Names from the target file are mapped to names from the current 
