...
\end{quote}
  We provide a concrete example in Sect.~\ref{sec:relat-interpr} below.
...
\item[Nonconservative extension]
  is represented by a theorem link (red arrow) in the development graph. 
  
\ednote{CL: show the Hets view syntax}
...
\item[Inclusion]
  ...
  \ednote{CL: show the Hets library (?) syntax} a list 
...
\ednote{CL: still need to port this to \HetCASL syntax}This example has two almost identical files, \texttt{upper.clif} and 
...
The sources are available from the \texttt{CommonLogic/Examples} directory in the \Hets library \cite{hets-library:URL}.
