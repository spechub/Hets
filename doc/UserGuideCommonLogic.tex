...
For use with \Hets, we have made a dump of the COLORE contents available in \texttt{CommonLogic/colore} in the \Hets library \cite{hets-library:URL}.

We first show how to express the relative interpretation using COLORE's CLIF annotations, then how to express it using \HetCASL syntax.  The \HetCASL variant can be found in the \texttt{CommonLogic/Examples} directory.

\paragraph{Common Logic Syntax:}\ednote{CL: TODO fix the second argument of ``relatively-interprets''}~\\
\begin{lstlisting}[language=clif]
...
\begin{lstlisting}[language=HetCASL,basicstyle={\small\ttfamily},alsolanguage=clif,alsoletter={-}]
library AAL
