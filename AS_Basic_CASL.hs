...
data BASIC_ITEMS = Sig_items SIG_ITEMS
		 ...
		 | Local_var_axioms [VAR_DECL] [FORMULA] [Pos]
		 | Axiom_items [AXIOM] [Pos]
		   deriving (Show,Eq)

data SIG_ITEMS = Sort_items [SORT_ITEM] [Pos] [Annotation]
	       | Op_items [OP_ITEM] [Pos] [Annotation]
	       | Pred_items [OP_ITEM] [Pos] [Annotation]
	       | Datatype_items [DATATYPE_DECL] [Annotation]
		 deriving (Show,Eq)

data SORT_ITEM = Sort_decl [SORT] [Pos] [Annotation]
	       | Sub_sort_decl [SORT] SORT [Pos] [Annotation]
	       | Subsort_defn SORT VAR SORT FORMULA [Pos] [Annotation]
	       | Iso_decl [SORT] [Pos] [Annotation]
		 deriving (Show,Eq)

data OP_ITEM = Op_decl [OP_NAME] OP_TYPE [OP_ATTR] [Pos] [Annotation]
	     | Op_defn OP_NAME OP_HEAD TERM [Pos] [Annotation]
	       deriving (Show,Eq)
...
data PRED_ITEM = Pred_decl [PRED_NAME] PRED_TYPE [Pos] [Annotation]
	       | Pred_defn PRED_NAME PRED_HEAD FORMULA [Pos] [Annotation]
		 deriving (Show,Eq)
...
data DATATYPE_DECL = Datatype_decl SORT [ALTERNATIVE] [Pos] 
		     deriving (Show,Eq)

data ALTERNATIVE = Total_construct OP_NAME [COMPONENTS] [Pos] [Annotation]
		 | Partial_construct OP_NAME [COMPONENTS] [Pos] [Annotation]
		 | Subsorts [SORT] [Pos] [Annotation]
		   deriving (Show,Eq)
...
data AXIOM = Axiom FORMULA [Pos] [Annotation] 
	     deriving (Show,Eq)
	     -- only used for Annotations and Pos of semicolon
