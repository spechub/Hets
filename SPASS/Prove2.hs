...
{- 
    todo:
      ...
      - merge SPASSResult into SPASSConfig
...
module SPASS.Prove2 (spassProver,spassProveGUI,spassProveBatch) where
...
openResult :: String -> SPASSConfig -> SPASSConfig
openResult n c = c{
    ...
    State {currentGoal = Nothing,
           ...
           configsMap = Map.fromList $
                        map (\ g -> let gName = AS_Anno.senName g
                                    in (gName, openResult gName emptyConfig)) goals,
...
-}
goalProcessed :: IORef SPASS.Prove2.State 
...
-}
updateDisplay :: SPASS.Prove2.State -- ^ current global prover state
              ...
              if cont
                 then batchProve lp' goalsProcessedSoFar' (res:resDone) gs
                              ...
                              (map (openSPASSProof_status .
                                  ...
                                  Map.findWithDefault (openResult gName emptyConfig)
                                     gName $ configsMap st)
                               lp)
...
runSpass :: SPLogicalPart -- ^ logical part containing the input Sign and axioms and possibly goals that have been proved earlier as additional axioms
         ...
         -> IO (SpassProverRetval, SPASSConfig) -- ^ (retval, (proof status, complete output))
      ...
      return (case excep of
                ...
                Exception.IOException e -> 
                    (SpassError ("Internal error communicating "++
                                 "with SPASS.\n"++show e),
                                openResult (AS_Anno.senName nGoal) cfg)
