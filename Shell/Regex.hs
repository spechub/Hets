...
showRegex _ (Label l _)      = l
showRegex _ Empty            = "%empty"
showRegex _ (Epsilon _)      = ""
showRegex _ (Terminal _ _ l) = l
showRegex _ (External _ l)   = l
...
showRegex _ (Star _ x)       = concat [showRegex StarContext x,"*"]
...
runRegex (Star f p) nwts = match [] nwts
  where match as ts =
...
strTerminal :: Char -> Regex Char [Char]
strTerminal ch = Terminal (==ch) (:[]) [ch]

strConcat :: forall a a1. Regex a1 [a] -> Regex a1 [a] -> Regex a1 [a]
strConcat      = Concat (++)

strAlt :: forall q a. Regex a q -> Regex a q -> Regex a q
strAlt         = altProj

strStar :: forall a a1. Regex a1 [a] -> Regex a1 [a]
strStar        = Star concat

strSpace :: Regex Char Char
strSpace       = Terminal (`elem` " \v\t\n\r\f") id ""

strAny :: forall a. Regex a [a]
strAny         = Terminal (const True) (:[]) ""

altProj :: forall q a. Regex a q -> Regex a q -> Regex a q
