...
import Shell.PPrint
import Shell.Regex (runRegex)
import Shell.ConsoleHandler
...
runShell desc backend nwinit = Ex.bracket setupShell exitShell (\iss -> shellLoop desc backend iss nwinit)
...
completionFunction desc backend bst st nline@(before,word,_) = do
   if all isSpace before
     then completeCommands desc nline
...
completeCommands desc (_,word,_) =
...
maximalPrefix (x:xs) = f x xs
  where f p [] = p
        f p (mx:nxs) = f (fst $ unzip $ takeWhile (\nx -> fst nx == snd nx) $ zip p mx) nxs
...
shellLoop desc backend iss ninit = loop ninit
 where
   ...
   handleSpecial st (ShellUse  ls)          = ( putStr $ show ls) >> loop st
   handleSpecial st (ShellHelp (Just ncmd))  = (outputString backend bst) (InfoOutput $ showCmdHelp desc ncmd) >> loop st
   ...
   handleExceptions ndesc f st = Ex.catch (f st) $ \ex ->
      runSh st (outputString backend bst) (exceptionHandler ndesc ex)
...
runSubshell _ (toSubSt, fromSubSt, mkSubDesc) backend _ st = do
  subSt   <- toSubSt st
