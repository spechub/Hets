...
getShellCommands :: forall st.ShellDescription st -> [(String, CommandParser st , Doc, Doc )]
...
showCmdHelp desc nwcmd =
  case cmds of
     [_] -> show (commandHelpDoc desc cmds) ++ "\n"
     _   -> show (text "bad command name: " <> squotes (text nwcmd)) ++ "\n"

 where cmds = filter (\ (n,_,_,_) -> n == nwcmd) (getShellCommands desc)


commandHelpDoc :: ShellDescription st ->  [(String,CommandParser st,Doc,Doc)] -> Doc

commandHelpDoc _ cmds =
...
--useCommand :: String -> f ->ShellCommand st
useCommand name f desc = (name
                        , doParseCommand Nothing (wordRegex (wordBreakChars desc)) id (wordBreakChars desc) f 
                        , text (maybePrefix desc)<> text name
                        , text "Use"
                       )
...
doParseCommand :: forall st f t x.
                  (CommandFunction f st) =>
                  Maybe (CommandCompleter st)
                  -> Regex Char x
                  -> (x -> t)
                  -> String
                  -> (t -> f)
                  -> [Char]
                  -> [CommandParseResult st]
doParseCommand compl _ _ _ _ []  = return (IncompleteParse compl)
