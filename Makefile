...
drifted_files = Common/AS_Annotation.hs \
    ...
    RigidCASL/AS_Rigid.hs TopHybrid/AS_TopHybrid.hs  \
...
atc_files = Common/AS_Annotation.der.hs Common/DefaultMorphism.hs \
    ...
    Logic/Prover.hs Logic/HDef.hs Logic/SemConstr.hs Common/LibName.hs Common/ExtSign.hs \
    ...
    Common/Lib/Graph.hs Common/IRI.hs Logic/SemConstr.hs Logic/HDef.hs
...
ATC/SemConstr.der.hs: Logic/SemConstr.hs $(GENRULES)
	$(GENRULECALL) -o $@ $<

ATC/HDef.der.hs: Logic/HDef.hs $(GENRULES)
	$(GENRULECALL) -i ATC.Id -i ATC.SemConstr -o $@ $<
...
ATC/AS_Library.der.hs: Syntax/AS_Library.der.hs $(GENRULES)
	$(GENRULECALL2) -i ATC.AS_Architecture -i ATC.LibName -i ATC.SemConstr -i ATC.HDef -o $@ $<
...
Rigid_files = RigidCASL/AS_Rigid.hs RigidCASL/Sign.hs
HPAR_files = HPAR/AS_Basic_HPAR.hs HPAR/Sign.hs HPAR/Morphism.hs HPAR/Symbol.hs
...
RigidCASL/ATC_RigidCASL.der.hs: $(Rigid_files) $(GENRULES)
	$(GENRULECALL) -i CASL.ATC_CASL -o $@ $(Rigid_files)

HPAR/ATC_HPAR.der.hs: $(HPAR_files) $(GENRULES)
	$(GENRULECALL) -i RigidCASL.ATC_RigidCASL -i CASL.ATC_CASL -o $@ $(HPAR_files)
...
GenHyb/ATC_Logic_Hyb.der.hs: $(Hyb_files) $(GENRULES)
	$(GENRULECALL)	-i ATC.AS_Annotation -i ATC.HDef -o $@ $(Hyb_files)
