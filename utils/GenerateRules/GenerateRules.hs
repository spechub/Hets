...
genRules flags files = 
    do i_ds <- mapM readParseFile files
       ...
       imports' <- checkImports (fileWP dir) 
                                ("Common.ATerm.Lib":imports)
       writeFile (fName dir)
                 (fileHead dir rule ds' ++ "\n\nmodule " ++ dir ++ "." 
                  ++ fileWP dir 
                  ++ " where\n\n" 
                  ++ (if dir == "CspCASL" then 
                  ...
                  ++ rules rule ds' ++ "\n")
    where 
    fName dir' = dir' ++ "/" ++ fileWP dir' ++ ".der.hs"
    fileHead dir' r ts = 
      "{- |\nModule      :  " ++ fName dir' ++
        ...
        "\n\nMaintainer  :  hets@tzi.de" ++
          "\nStability   :  provisional" ++
          "\nPortability :  portable\n" ++
          "\n  Automatic derivation of instances via DrIFT-rule " ++ r ++ 
          "\n  for the type(s):" ++ concatMap ( \ t -> " '" ++ t ++ "'") ts ++          "\n-}\n" ++ "{-\n  Generated by 'genRules' " ++ 
          "(automatic rule generation for DrIFT). Don't touch!!" ++ 
          "\n  dependency files: " ++ unwords files ++ "\n-}"
    fileWP dir = if (firstdirname (head files) /= (dir++"/")) 
