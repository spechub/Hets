-- |
-- n-ary tree structure with filter combinators
...
module Data.NTree
    (
      -- * The NTree n-ary Tree Data Type
      NTree(..)
    , NTrees
    , TFilter
    , TSFilter,
    ...
    , mkNode
      ...
      -- * Selector Functions and Predicates
    , getNode		-- selectors
    , getChildren

    , satisfies		-- predicate

    , formatNTree,	-- trace output
    ...
    , foldNTree		-- general NTree functions
    , mapNTree
    , nTreeToList
    , depthNTree
    , cardNTree
...
-- | n-ary ordered tree with nodes of type node
--
...
-- |
-- construct a leaf, a tree with an empty list of children
--
--    * 1.parameter n :  the node value
--
--    - returns : the leaf containing the value n

mkLeaf		:: node ->                NTree node
mkLeaf n	= NTree n []
...
--    * 1.parameter n :  the node value
--
--    - 2.parameter cs :  the list of children
--
--    - returns : the tree

mkNode		:: node -> NTrees node -> NTree node
mkNode n cs	= NTree n cs

-- selector functions

-- |
-- selector function for the node value of a tree
--
--    * 1.parameter t :  the tree
--
--    - returns : the node
...
-- |
-- selector function for the children of a tree
--
--    * 1.parameter t :  the tree
--
...
-- format tree
--
-- a /graphical/ representation of the tree in text format is generated
--
--    * 1.parameter nf :  the conversion function from nodes to text
--
--    - 2.parameter t :  the tree
--
--    - returns : the formatted text

formatNTree	:: (node -> String) -> NTree node -> String

formatNTree node2String n
    = formatNTreeF node2String (showString "---") (showString "   ") n ""

-- ------------------------------------------------------------
-- |
-- formating done with function composition
-- like in standard prelude in class Show
...
foldNTree f (NTree n cs)
    = f n (map (foldNTree f) cs)

-- |
-- map for NTree

mapNTree	:: (a -> b) -> NTree a -> NTree b
mapNTree f (NTree n cs)
    = NTree (f n) (map (mapNTree f) cs)

-- |
-- all nodes of a tree in preorder

nTreeToList	:: NTree a -> [a]
nTreeToList	= foldNTree (\ n rs -> n : concat rs)

-- |
-- depth of a tree
		     
depthNTree	:: NTree a -> Int
depthNTree	= foldNTree (\ _ rs -> 1 + maximum (0 : rs))

-- |
-- number of nodes in a tree

cardNTree	:: NTree a -> Int
cardNTree	= foldNTree (\ _ rs -> 1 + sum rs)
