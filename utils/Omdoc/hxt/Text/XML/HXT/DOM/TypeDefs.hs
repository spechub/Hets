...
-- | Equality of QNames: Two QNames are equal if namespace uris are equal and local parts are equal

equalQName	:: QName -> QName -> Bool
equalQName 	= equalQNameBy id

-- | Sometimes a weaker equality relation than 'equalQName' is appropriate, e.g no case significance in names, ...
-- a name normalization function can be applied to the strings before comparing

equalQNameBy	:: (String -> String) -> QName -> QName -> Bool
equalQNameBy normalize q1 q2
    = normalize (localPart q1) == normalize (localPart q2)
      &&
      normalize (namespaceUri q1) == normalize (namespaceUri q2)
