...
userCode s = let current = splitString codeSeperator s
		 old = splitString old_codeSeperator s
	     in if "{-* Generated by DrIFT" `isPrefixOf` snd old -- -} for emacs!!
	        then old else current
codeSeperator = "{- ? Generated by DrIFT : Look, but Don't Touch. (works w/ haddock) ? -}"
old_codeSeperator = "{-* Generated by DrIFT : Look, but Don't Touch. *-}"
