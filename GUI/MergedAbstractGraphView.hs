...
data AbstractionGraph = AbstractionGraph {
       ...
       edgeTypes :: [(String,DaVinciArcType a)],
       nodes :: [(Int,(String,DaVinciNode (String,Int,Int)))],
       edges :: [(Int,(Int,Int,String,DaVinciArc a))],
...
data Entry = Entry {newNodes :: [(Descr,(String,
                    ...
                    newEdges :: [(Int,(Int,Int,String,DaVinciArc a))],
                    oldEdges :: [(Int,(Int,Int,String,a))]
                    }
...
createEntry :: [(Descr,(String,DaVinciNode (String,Int,Int)))]
            -> [(Descr,(String,String))]
            -> [(Descr,(Int,Int,String,DaVinciArc a))]
            -> [(Descr,(Int,Int,String,a))] -> Descr -> (Int,Entry)
      ...
      (Just et, Just src_node, Just tar_node) -> do
        existingEdgesOfSameTypeAndPosition <- 
          sequence [(getArcValue (theGraph g) davinciArc)
                    |(descr,(srcId, tgtId, tp, davinciArc)) <- (edges g),
                    tp == edgetype && srcId == src && tgtId == tar]
        case lookup name [(nm,descr)|(nm,descr,graphId) <- 
...
fetchEdgesOfNode g node = 
  case sequence (map ((flip lookup) (edges g)) (map fst (edges g))) of
    Just el -> 
      Just ([descr|ed@(descr,(_,t,_,_)) <- (edges g), t == node],
            [descr|ed@(descr,(s,_,_,_)) <- (edges g), s == node])
    Nothing -> Nothing
              ...
              Nothing -> do
                ...
                case error2 of
                  Nothing -> do
                    ...
                    -- ... then try to remove them from the graph
                    deletedNodes@(Result de3 error3) <- 
                    ...
                    case error3 of
                      Nothing -> do
                        ...
                        let 
                          ...
                          newEvent = 
                            createEntry [] oldNodes newEdges oldEdges ev_cnt
          ...
          Nothing -> 
            return (g,0,ev_cnt,
                    Just "hidenodes: error making paths\n(possible reasons: an"
                         ++" error occured getting the edges of the nodes\nor "
                         ++"a pathtype could not be determined (missing entry "
                         ++"in edgeComp table))")
...
hidenodesaux gid [] gv = do
  (gs,ev_cnt) <- readIORef gv
  return (Result ev_cnt Nothing)
hidenodesaux gid (d:delNodes) gv = do
  deletedNode@(Result de error) <- delnode gid d gv
  case error of
...
makepathsaux :: AbstractionGraph -> [Descr] -> [Descr]
             -> ((Descr,Descr,String,DaVinciArc a),
                (Descr,Descr,String,DaVinciArc a))
             -> Maybe [(Descr,Descr,String)]
...
addpaths gid ((src,tgt,ty):newEdges) gv = do
  edge@(Result de error) <- addlink gid ty "" Nothing src tgt gv
  case error of
        ...
        case sequence (map (fetchEdgesOfNode g) node_list) of
          Just el -> do
            ...
            {- try to create the new abstract node and add its in- and outgoing
               paths -}
            (Result de1 error1) <- replaceByAbstractNode gid node_list nl 
            ...
            case error1 of
              Nothing -> do
                ...
                case error2 of
                  Nothing -> do
                    ...
                    (Result de3 error3) <- hidenodesaux gid node_list gv
                                             --info2
                    info3 <- readIORef gv
                    case error3 of
                      Nothing -> do
              ...
              Just t ->
                return (g,0,ev_cnt,Just ("abstractnodes: error making abstract"                                          ++" node: "++ t))
          Nothing -> 
...
hideedgesaux gid [] gv = do
  (gs,ev_cnt) <- readIORef gv
  return (Result ev_cnt Nothing)
hideedgesaux gid (d:delEdges) gv = do
  dle@(Result descr error) <- dellink gid d gv
  case error of
          ...
          Nothing -> do
            ...
            case error2 of
              Nothing -> do
                ...
                (Result de3 error3) <- hidenodesaux gid
...
shownodes gid [] gv = do 
  (gs,ev_cnt) <- readIORef gv
  return (Result ev_cnt Nothing)
shownodes gid ((node@(d,(tp,name))):list) gv = do
  (gs,_) <- readIORef gv
...
showedges :: Descr -> [(Int,(Int,Int,String,a))] -> GraphInfo
          -> IO Result
showedges gid [] gv = do
  (gs,ev_cnt) <- readIORef gv
  return (Result ev_cnt Nothing)
showedges gid ((edge@(d,(src,tgt,tp,value))):list) gv = do
  (gs,_) <- readIORef gv
  let g = snd (get gid gs)
...
saveOldNodes g ((node@(de,(tp,davincinode))):list) = do
  (name,descr,gid) <- getNodeValue (theGraph g) davincinode
...
saveOldEdges :: AbstractionGraph
             -> [(Int,(Int,Int,String,DaVinciArc a))]
             -> IO [(Int,(Int,Int,String,a))]
saveOldEdges g [] = return []
saveOldEdges g (edge@(de,(src,tgt,tp,davinciarc)):list) = do
