...
isHiddenNode gi nId = do
  g <- readIORef gi
  return $ isNothing $ udgNode $ get nId $ nodes g
...
addEdge gi eId eType nIdFrom nIdTo eName eLabel = do
  ...
  edge' <- case getEdgeForAdding g nIdFrom nIdTo eType of
      Just (nFrom, nTo, gae) ->
          fmap Just $ newArc (theGraph g) (udgEdgeType gae) gaeV nFrom nTo
...
getEdgeForAdding :: AbstractionGraph -> NodeId -> NodeId -> DGEdgeType
  -> Maybe (DaVinciNode NodeValue, DaVinciNode NodeValue, GAEdgeType)
getEdgeForAdding g nIdFrom nIdTo eType =
  let ns = nodes g
      gae = get eType $ edgeTypes g
  in case (udgNode $ get nIdFrom ns, udgNode $ get nIdTo ns) of
    (Just nFrom', Just nTo') | gaeHidden gae -> Just (nFrom', nTo', gae)
    _ -> Nothing
...
addCompressedEdge gi (nIdFrom, nIdTo, eType) = do
  ...
  edge' <- case getEdgeForAdding g nIdFrom nIdTo eType of
      Just (nFrom, nTo, gae) ->
          fmap Just $ newArc (theGraph g) (udgCompressed gae) gaeV nFrom nTo
      Nothing -> return Nothing
  let edge = GAEdge { udgEdge = edge'
                    , gaeType = eType
                    , ganFrom = nIdFrom
        ...
        Just eType' -> do
          case udgEdge edge of
            Nothing -> writeIORef gi g
              { edges = Map.insert eId edge { gaeType = eType } $ edges g }
            Just edge' -> do
              deleteArc (theGraph g) edge'
              ...
              e <- newArc (theGraph g) (udgEdgeType $ eType') (gaeValue edge)
                          from to
              writeIORef gi g
                { edges = Map.insert eId edge
                          { udgEdge = Just e
                          , gaeType = eType } $ edges g }
