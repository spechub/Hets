...
mkAttr l = case l of
  ...
  _ -> Fail.fail $ "XPATH unexpected attr: " ++ show l
...
changeXml el input = case parseXMLDoc input of
    Nothing -> Fail.fail "changeXml: cannot parse xupdate file"
...
changeXmlMod el diff = let cr = fromElement el in do
  ...
  case current cr' of
     Elem e -> return (e, chL)
     _ -> Fail.fail "unexpected content within top element"
...
applyChanges changes cr dir chL = do
  ...
  cr' <- case cres2 of
      ...
      RemoveCr -> case dir of
        ...
        TopElem -> Fail.fail "Top Element cannot be removed!"
  return (cr', chL')
...
applyChange (ChangeCr cr) (ChangeData csel attrSel) = case (csel, attrSel) of
  ...
  _ -> Fail.fail $ "no implementation for :" ++ show csel
...
changeText t cr = case current cr of
  Elem e | null $ onlyElems $ elContent e -> return $ ChangeCr cr
    { current = Elem $ e
      { elContent = [mkText t] }}
  _ -> Fail.fail "current cursor is no element with text only"
...
removeOrChangeAttr csel cr atS =
  let failMsg msg = Fail.fail $ "removeOrChangeAttr '" ++ atS ++ "': " ++ msg
...
applyAddOp pos cr addCh = case (pos, addCh) of
        ...
        (Append, AddElem e) -> case current cr of
            ...
            _ -> Fail.fail "applyAddOp: unexpected content(1)"
        (Append, AddAttr at) -> case current cr of
            Elem e -> return cr { current = Elem $ add_attr at e }
            _ -> Fail.fail "applyAddOp: unexpected content(2)"
        _ -> Fail.fail "applyAddOp: illegal addChange-data!"
