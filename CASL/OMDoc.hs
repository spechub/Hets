...
caslMetaTheory :: OMCD
caslMetaTheory = CD "casl" $ Just "http://cds.omdoc.org/logics/casl.omdoc"
...
exportSubsortToOmdoc spid (s1, s2) = Just $ TCSymbol
...
exportSenToOmdoc sid lid sign f =
    let spid  = (SPEC_ID (simpleIdToId sid) (libIdToId lid))
        ...
        sen   = sentence f
        (role, defn) =
            if isAxiom f then (Axiom, Nothing)
            else (Theorem, Just const_unproven)
    in
      case sen of
        Sort_gen_ax cs b -> makeADTsFromConstraints spid sign cs
...
makeADTsFromConstraints :: SPEC_ID -> Sign f e -> [Constraint] -> ADTType
                         -> TCElement
makeADTsFromConstraints spid _ cs b =
    TCADT $ Prelude.map (makeADTSortDef spid b) cs

makeADTSortDef :: SPEC_ID -> ADTType -> Constraint -> OmdADT
...
sortSignToOmdoc spid _ s
    | isLocalId spid s =
        Just $ TCSymbol (idToName spid s) const_sort Typ Nothing
...
predSignToOmdoc spid _ p ptypes
    | isLocalId spid p =
        Just $ TCSymbol (idToName spid p)
                 (makePredType spid $ Set.findMin ptypes) Obj Nothing
