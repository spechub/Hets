        ...
        do e <- get -- save
           ...
           let (es, resFs, anaFs) = foldr ( \ f (dss, ress, ranas) ->
               ...
               fufs = map (mapAn (\ f -> let
                      ...
                      anaFs
               sens = map ( \a -> (emptyName (getRLabel a) $ item a) {
...
toSortGenAx ps isFree (sorts, rel, ops) = do
    ...
    addSentences [emptyName ("ga_generated_" ++
    ...
    Op_defn i ohd at ps ->
        do let ty = headToType ohd
           ...
           case mt of
             ...
             Just (resT, anaT) -> do
                 let p = posOfId i
                 addSentences [(emptyName lab $ mkForall vs
                     (Strong_equation
...
makeUnit b t ty ni i =
    ...
    in (emptyName lab $ mkForall [Var_decl [v] vty q]
                     (Strong_equation
...
makeDisjSubsorts d subs = case subs of
  ...
  makeDisjSubsort s1 s2 = let
     ...
     v = Var_decl [n] d pd
     qv = toQualVar v
     in emptyName ("ga_disjoint_sorts_" ++ showId s1 "_" ++ showId s2 "")
...
makeDisjToSort a s =
    let (c, v, t, _) = selForms1 "X" a
        p = posOfId s
    in emptyName ("ga_disjoint_" ++ showId c "_sort_" ++ showId s "")
...
makeInjective a =
    let (c, v1, t1, _) = selForms1 "X" a
        ...
        p = posOfId c
    in emptyName ("ga_injective_" ++ showId c "")
        $ mkForall (v1 ++ v2)
...
makeDisjoint l = case l of
  ...
  makeDisj a1 a2 =
    ...
    in emptyName ("ga_disjoint_" ++ showId c1 "_" ++ showId c2 "")
...
makeUndefForm (s, ty) (i, vs, t, sels) =
    ...
    then Nothing else
       Just $ emptyName ("ga_selector_undef_" ++ showId s "_" ++ showId i "")
              $ mkForall vs
               (Negation
                (Definedness
...
makeSelForms n (i, vs, t, (mi, ty):rs) =
    (case mi of
            ...
            Just j -> let p = posOfId j
                          rty = opRes ty
                          q = posOfId rty in
              [emptyName ("ga_selector_" ++ showId j "")
                     $ mkForall vs
                      (Strong_equation
