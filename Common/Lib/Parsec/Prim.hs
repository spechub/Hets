...
consumeNothing = Parser (\state -> Consumed (Ok () state (unknownError state)))
...
manyAccum accum (Parser p)
  = Parser (\state -> 
    let walk xs state r = case r of
                           ...
                           Empty ok                   -> error "Common.Lib.Parsec.Prim.many: combinator 'many' is applied to a parser that accepts an empty string."
    ...
    in case (p state) of
         Empty reply  -> case reply of
                           Ok x state' err -> error "Common.Lib.Parsec.Prim.many: combinator 'many' is applied to a parser that accepts an empty string."
