...
-- operations
   ufoldM,                              -- graph fold
   nodesM,edgesM,labEdgesM,newNodesM,   -- graph projection
   delNodeM,delNodesM,                  -- graph construction & destruction
   mkUGraphM,
   contextM,labM                        -- graph inspection
)
where
...
class Monad m => GraphM m gr where
  ...
  labEdgesM = ufoldM (\(p,v,l,s)->(((map (inn v) p)++(map (out v) s))++)) []
              where out v = \(l,w)->(v,w,l)
                    inn v = \(l,w)->(w,v,l)
