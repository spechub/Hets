...
bfForEK q g
    ...
    | otherwise                 = case match v g of
        ...
        (Just c@(preAdj, _, _, sucAdj), g') -> p:bfForEK q2 g'
            where
...
extractPathFused g [(u,_)] = ([], g)
extractPathFused g ((u,_):rest@((v,Forward):ws)) =
    ((u, v, lab, Forward):tailedges, newerg)
        where (tailedges, newerg) = extractPathFused newg rest
              Just (lab, newg)    = extractEdge g u v (\(c,f)->(c>f))
extractPathFused g ((u,_):rest@((v,Backward):ws)) =
    ((v, u, lab, Backward):tailedges, newerg)
        where (tailedges, newerg) = extractPathFused newg rest
              Just (lab, newg)    = extractEdge g v u (\(c,f)->(f>0))

-- ekFusedStep :: EKStepFunc
ekFusedStep g s t = case maybePath of
        Just path -> 
            Just ((insEdges (integrateDelta edges delta) newg), delta)
        Nothing   -> Nothing
    where maybePath     = augPathFused g s t
          (edges, newg) = extractPathFused g (fromJust maybePath)
          delta         = minimum $ getPathDeltas edges
...
extractPath g (u:v:ws) =
    case fwdExtract of
        Just (lab, newg) -> ((u, v, lab, Forward):tailedges, newerg)
    ...
    where fwdExtract = extractEdge g u v (\(c,f)->(c>f))
          revExtract = extractEdge g v u (\(c,f)->(f>0))
...
extractEdge g u v p =
    case adj of
        Just (el, w) -> Just (el, (pre, node, lab, rest) & newg)
        Nothing      -> Nothing
    where (Just (pre, node, lab, suc), newg) = match u g
          (adj, rest)=extractAdj suc 
              (\(lab, dest) -> (dest==v) && (p lab))
...
getPathDeltas (e:es) = case e of
    (u, v, (c,f), Forward)  -> (c-f) : (getPathDeltas es)
    (u, v, (c,f), Backward) -> f : (getPathDeltas es)
...
ekSimpleStep g s t = case maybePath of
    ...
    where maybePath     = augPath g s t
          (edges, newg) = extractPath g (fromJust maybePath)
          delta         = minimum $ getPathDeltas edges
...
setContains map i = case (lookupFM map i) of
    Nothing -> False
    Just () -> True
...
extractPathList []               set = ([], [])
extractPathList (edge@(u,v,lab@(c,f)):es) set
    | (c>f) && (setContains set (u,v)) = 
        let (pathrest, notrest)=extractPathList es (delFromFM set (u,v))
            in ((u,v,lab,Forward):pathrest, notrest)
    | (f>0) && (setContains set (v,u)) =
        let (pathrest, notrest)=extractPathList es (delFromFM set (u,v))
            in ((u,v,lab,Backward):pathrest, notrest)
    | otherwise                        =
...
ekStepList g s t = case maybePath of
        Just path -> Just (mkGraph (labNodes g) newEdges, delta)
        Nothing   -> Nothing
    where newEdges      = (integrateDelta edges delta) ++ otheredges
          maybePath     = augPathFused g s t
          (edges, otheredges) = extractPathList (labEdges g) 
