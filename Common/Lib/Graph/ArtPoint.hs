{- 
   This module finds the articulation points for a connected undirected graph,
...
dfsTree n u vs     ls g | null vs || isEmpty g = ([],g,n)
...
getLowTree g v = lowTree (head dfsf)
                  where (dfsf, g1, m) = dfsTree 0 0 [v] [] g
...
isap (Brc (v,n,l) []) = False
isap (Brc (v,1,l) ts) = length ts > 1
isap (Brc (v,n,l) ts) = length ch >= 1
...
arp (Brc (v,1,l) ts) | length ts > 1         = v:concatMap arp ts
...
------------------------------------------------------------------------------
-- Finds the articulation points of a graph starting at an arbitrary node.
------------------------------------------------------------------------------
ap :: Graph gr => gr a b -> [Node]
ap g = artpoints g v where ((p,v,l,s),g') = matchAny g
