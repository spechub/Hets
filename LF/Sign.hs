...
getSymbols :: Sign -> Set.HashSet Symbol
...
getDeclaredSyms :: Sign -> Set.HashSet Symbol
...
-- get the set of declared symbols
getDefinedSyms :: Sign -> Set.HashSet Symbol
...
getLocalSyms :: Sign -> Set.HashSet Symbol
...
getGlobalSyms :: Sign -> Set.HashSet Symbol
...
getSymsOfType :: EXP -> Sign -> Set.HashSet Symbol
getSymsOfType t sig =
...
{- modifies the given name until it is different from each of the names
   in the input set -}
getNewName :: VAR -> Set.HashSet VAR -> VAR
getNewName var names = getNewNameH var names var 0

getNewNameH :: VAR -> Set.HashSet VAR -> VAR -> Int -> VAR
...
getFreeVars :: EXP -> Set.HashSet VAR
getFreeVars e = getFreeVarsH $ recForm e

getFreeVarsH :: EXP -> Set.HashSet VAR
...
getConstants :: EXP -> Set.HashSet Symbol
getConstants e = getConstantsH $ recForm e

getConstantsH :: EXP -> Set.HashSet Symbol
...
rename :: Map.HashMap VAR VAR -> Set.HashSet VAR -> EXP -> EXP
rename m s e = renameH m s (recForm e)

renameH :: Map.HashMap VAR VAR -> Set.HashSet VAR -> EXP -> EXP
...
coGenSig :: Set.HashSet Symbol -> Sign -> Result Sign
coGenSig syms sig = do
  let syms' = exclSyms syms sig
  let defs' = filter (\ d -> HSU.notMember (getSym d) syms') $ getDefs sig
  return $ Sign gen_base gen_module defs'

exclSyms :: Set.HashSet Symbol -> Sign -> Set.HashSet Symbol
exclSyms syms sig =
  foldl (\ syms' (Def s t v) ->
