...
parseLogic :: LogicGraph -> AParser (Logic_code,AnyLogic)
parseLogic lG = 
    do l <- asKey logicS
...
parseLogAfterColon :: LogicGraph -> Maybe Token -> [Token] -> AParser (Logic_code,AnyLogic)
...
parseMapping :: (AnyLogic, LogicGraph) 
                  -> AParser ([G_mapping], [Token], AnyLogic)
