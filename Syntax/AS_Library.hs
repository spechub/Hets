...
instance GetRange ITEM_NAME_OR_MAP where
  getRange x = case x of
    Item_name _ -> nullRange
    Item_name_map _ _ p -> p
  rangeSpan x = case x of
    Item_name a -> joinRanges [rangeSpan a]
    Item_name_map a b c -> joinRanges [rangeSpan a, rangeSpan b,
                                       rangeSpan c]
