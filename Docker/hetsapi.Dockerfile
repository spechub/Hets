...
# RUN git clone https://github.com/spechub/Hets.git /opt/hets
## OR
COPY ./ /opt/hets
...
RUN git clone https://github.com/spechub/Hets-lib.git /opt/Hets-lib
ENV HETS_LIB=/opt/Hets-lib
ENV PYTHONPATH=/opt/hets/python/

RUN useradd -ms /bin/bash -u 921 hets
USER hets
