...
{-
  Interface between CspCASLProver and Hets:
   Hets writes CspCASLProver's Isabelle .thy file and starts Isabelle with CspProver
   User extends .thy file with proofs
   User finishes Isabelle
   Hets reads in created *.deps files
-}

module CspCASLProver.CspCASLProver where
...
openIsaProof_status n = openProof_status n cspCASLProverS ()
...
cspCASLProverProve thName th _freedefs = return []
