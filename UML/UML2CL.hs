...
{-
 (foldl (++) [] $ (map translateCompLabel compositions))
            ...
            compositions = filter isComposition $ Map.elems $ cmAssociations cm
-}
translateBinAsso :: (String,[(String,Type)]) -> [PHRASE]
...
translateEndCoin :: String -> Integer -> (String,Type) -> Maybe PHRASE
...
translateAssociation :: (String,[(String,Type)]) -> [PHRASE]
translateAssociation (n,endL)  = [    Sentence $ typeFunction (mkSimpleId "form:Sequence") [mkSimpleId n],
...
translateInherit (cl,superc) =   (Sentence (Quant_sent Universal [Name t]  (Bool_sent (BinOp Implication 
...
translateAttribute :: (Class,String,Type) -> [PHRASE]
translateAttribute (cl,n,t) = mem ++ (map (translateProperty attrt l) sens) 
            where     
                x = mkSimpleId "x"
                y = mkSimpleId "y"
                l = [(c,x),(cp,y)]
...
translateProcedure :: (Class,String,[(String,Type)],Type) -> [PHRASE]
translateProcedure (cl,n,paras,t) = mem ++ (map (translateProperty proct l) sens)
            where     
