...
preamble = [	Sentence (termAtom (Name_term dt )),
		...
		where 	dt = (mkSimpleId "distinct")
			bumlStr = mkSimpleId "buml:String"
			fip = mkSimpleId "form:in-position"
			fis = mkSimpleId "form:in-sequence"
			fP = mkSimpleId "form:Position"
			fS = mkSimpleId "form:Sequence"
			x = mkSimpleId "x"
			x1 = mkSimpleId "x2"
			x2 = mkSimpleId "x1"			
			y = mkSimpleId "y"
...
translateClasses :: Map.Map UML.Id UML.Class -> [PHRASE]
translateClasses cmap =   foldl (++) [] $ map translateClass (Map.elems cmap)

translateClass :: UML.Class -> [PHRASE]
...
leftTotallityRule c l (cz,z) = Sentence (Quant_sent Universal (map (\(a,b) -> Name b) (delete (cz,z) l)) (Quant_sent Existential [Name z] (termAtom (typeFunction c (map snd l))) nullRange) nullRange)
...
translateCompLabel2 r (f, l)  = Sentence (Quant_sent Universal [Name x, Name y, Name n] (Bool_sent (BinOp Implication (Bool_sent (Junction Conjunction [termAtom $ typeFunction r [x,y], termAtom $ typeFunction (mkSimpleId "form:sequence-length") [y,n]]) nullRange) (termAtom $ typeFunction f [l,n])) nullRange) nullRange) 
	where 
		x = mkSimpleId "x" 
		y = mkSimpleId "y" 
		n = mkSimpleId "n" 
...
translateAssoLabel2 a xcl (sels,f)=  Sentence (Quant_sent Universal (map (\(x,c)-> Name x) xcl) (Bool_sent (BinOp Implication (Bool_sent (Junction Conjunction (map (\(x,c) -> termAtom$ typeFunction c [x]) xcl)) nullRange) (termAtom $ Funct_term (typeFunction (mkSimpleId f) (a : (map fst xcl))) (map (\t -> Term_seq (Name_term $ t)) sels) nullRange)) nullRange) nullRange)

rightUniqueRule ::  Token -> [(Token,Token)] -> (Token,Token) -> Token -> PHRASE
rightUniqueRule c l (cy,y) z = Sentence (Quant_sent Universal ((map (\(a,b) -> Name b) l) ++ [Name z]) (Bool_sent (BinOp Implication (Bool_sent (Junction Conjunction [(termAtom (typeFunction c (map snd l))),(termAtom (typeFunction c (((delete y (map snd l)) ++[z]))))]) nullRange) (termAtom (typeFunction (mkSimpleId "=") [y,z])) ) nullRange) nullRange)

memberRule :: Token -> [(Token,Token)] -> (Token, Token) -> Token -> PHRASE
memberRule c l (cy,y) m = Sentence (Quant_sent Universal (map (\(a,b) -> Name b) (l++[(m,m)])) (Bool_sent (BinOp Implication (Bool_sent (Junction Conjunction [(termAtom (typeFunction c (map snd l))), termAtom (typeFunction (mkSimpleId "member") [m,y] )] ) nullRange ) (termAtom (typeFunction cy [m])) ) nullRange) nullRange) 
