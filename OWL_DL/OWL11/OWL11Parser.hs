...
main = do args <- getArgs
          if (null args) || (length args > 1) then
       ...
       where isURI :: String -> Bool
             ...
             process args  =
                 do
                  pwd <- getEnv "PWD"
                  if isURI $ head args then
                      do res <- parseOWL $ head args
                         putStrLn $ show res
                    else if (head $ head args) == '/' then
                          do res <- parseOWL ("file://" ++ head args)
                             putStrLn $ show res
                           else do res <- parseOWL ("file://" ++ pwd ++ "/" ++ head args)
                                   putStrLn $ show res
       ...
       then
         error "empty file name!"
           ...
           if checkUri filename
               then
                 do exitCode <-
                        system ("$HETS_OWL_PARSER/OWL11/owl_parser " ++ filename ++
                               " " ++ tmpFile)
       ...
       where buildTime cTime =
             ...
             run exitCode tmpFile
                 | exitCode == ExitSuccess =
                     do
                       putStrLn ("ATerm-File has been saved: "  ++ tmpFile)
                       t <- parseProc tmpFile
                       -- system ("rm -f " ++ tmpFile)
                       return t
