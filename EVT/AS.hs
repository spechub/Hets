...
instance (GetRange g, GetRange a) => GetRange (MACHINE g a) where
  getRange x = case x of
    MACHINE _ _ p -> p
  rangeSpan x = case x of
    MACHINE a b c -> joinRanges [rangeSpan a, rangeSpan b, rangeSpan c]

instance (GetRange g, GetRange a) => GetRange (EVENT g a) where
  getRange = const nullRange
  rangeSpan x = case x of
    EVENT a b c -> joinRanges [rangeSpan a, rangeSpan b, rangeSpan c]

instance GetRange g => GetRange (GUARD g) where
  getRange = const nullRange
  rangeSpan x = case x of
    GUARD a b -> joinRanges [rangeSpan a, rangeSpan b]

instance GetRange a => GetRange (ACTION a) where
  getRange = const nullRange
  rangeSpan x = case x of
