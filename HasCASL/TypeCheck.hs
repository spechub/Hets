                              ...
                              x -> let br = case x of
                                      ...
                                      (s, cs, ty, 
                                       QualOp br (InstOpId i is [])
                                                  (opType oi) ps)) ls
            else infer b mt $ ApplTerm (ResolvedMixTerm i [] ps)
        ...
        TupleTerm ts ps -> 
            ...
            Just ty -> do 
                ...
                case ms of 
                     Nothing -> return []
                     Just s  -> do 
                         ls <- checkList b (map (Just . subst s) vs) ts 
                         return $ map ( \ (su, cs, tys, trms) ->
...
inferLetEqs es = do
    ...
    do as <- gets assumps 
       ...
       combs <- mapM ( \ (sf, _, tys, pps) -> do
             ...
             newTrms <- checkList True (map Just tys) trms 
