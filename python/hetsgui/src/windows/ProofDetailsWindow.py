...
class ProofDetailsWindow(Gtk.Window):
    ...
    def __init__(self, goal: Sentence, theory: Theory, **kwargs):
        ...
        for comorphism, proof in goal.theorem_status():
            ...
            # proof_detail.goal_status = proof.kind()
            # proof_detail.used_prover = proof.details().used_prover()
            # proof_detail.used_comorphism = comorphism.name()
            # proof_detail.used_time = proof.details().used_time()
            # proof_detail.used_tactic_script = proof.details().tactic_script()
        ...
        # """)
        # self.get_style_context().add_provider_for_screen(self.get_screen(), provider,
        ...
        #     expander_label.set_tooltip_text(
        #         f"{proof.kind().to_str()} with {proof.details().used_prover()} via {comorphism.name()}")
        #     expander_label.set_markup(
        #         f"<span foreground='{PROOF_KIND_FG_COLORS[proof.kind()]}'>{proof.kind().to_str()}</span> with <b>{proof.details().used_prover()}</b> via <b>{comorphism.name()}</b>")
        ...
        #     details_grid.attach(Gtk.Label(
        #         label=f"<span foreground='{PROOF_KIND_FG_COLORS[proof.kind()]}'>{proof.kind().to_str()}</span>",
        #         use_markup=True, halign=Gtk.Align.START), 1, row, 1, 1)
        #     row += 1
        #
        ...
        #     details_grid.attach(
        #         Gtk.Label(label=comorphism.name(), halign=Gtk.Align.START, ellipsize=Pango.EllipsizeMode.MIDDLE,
        #                   tooltip_text=comorphism.name()), 1, row, 1, 1)
        #     row += 1
        #
        ...
        #         details_grid.attach(Gtk.Label(label=proof.details().tactic_script(), halign=Gtk.Align.START), 1, row, 1,
        #                             1)
        #         row += 1
        #
        ...
        #     proof_lines_widget = Gtk.Label(label="\n".join(proof.details().proof_lines()), halign=Gtk.Align.START,
        #                                    selectable=True, lines=len(proof.details().proof_lines()), wrap=True)
