...
Also look into [programming guidelines](http://www.haskell.org/haskellwiki/Programming_guidelines) anhd [things to avoid in Haskell](http://www.haskell.org/haskellwiki/Things_to_avoid).
...
* Use `scan` which can be installed be install by `cabal install scan`.
