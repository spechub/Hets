...
module OWL2.ManchesterPrint ({-printOWLBasicTheory,-} printAxiom) where
...
printClassFrameBit cfb = case cfb of
    ClassAnnotations x -> printAnnotations x
    ClassSubClassOf x -> printAnnotatedList x
    ClassEquivOrDisjoint x y -> printEquivOrDisjoint x <+> printAnnotatedList y
    ClassDisjointUnion a x -> keyword disjointUnionOfC <+> printAnnotations a <+> setToDocV (Set.fromList x)
...
printObjectFrameBit ofb = case ofb of
    ObjectAnnotations x -> printAnnotations x
    ObjectDomainOrRange dr x -> printObjDomainOrRange dr <+> printAnnotatedList x
    ObjectCharacteristics x -> printAnnotatedList x
    ObjectEquivOrDisjoint ed x -> printEquivOrDisjoint ed <+> printAnnotatedList x
    ObjectInverse x -> keyword inverseOfC <+> printAnnotatedList x
    ObjectSubPropertyChain a opl -> keyword subPropertyChainC <+> printAnnotations a
