...
instance Comorphism CoCFOL2IsabelleHOL
    ...
    targetSublogic CoCFOL2IsabelleHOL = ()
    map_theory CoCFOL2IsabelleHOL = transTheory sigTrCoCASL formTrCoCASL
    map_morphism CoCFOL2IsabelleHOL mor = do
       (sig1,_) <- map_sign CoCFOL2IsabelleHOL (Logic.dom CoCASL mor)
       (sig2,_) <- map_sign CoCFOL2IsabelleHOL (cod CoCASL mor)
       inclusion Isabelle sig1 sig2
    map_sentence CoCFOL2IsabelleHOL sign =
      return . mapSen formTrCoCASL sign
    map_symbol CoCFOL2IsabelleHOL _ = error "CoCFOL2IsabelleHOL.map_symbol"
