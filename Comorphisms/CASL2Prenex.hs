...
-- turn arbitrary junctions into binary ones
 ...
 Negation nsen _ -> let
   ...
   (n'', nsen'') = case nsen' of
    Quantification q vdecls qsen _ -> -- deMorgan
       (n', Quantification (dualQuant q) vdecls
              (Negation qsen nullRange) nullRange)
   ...
   (n3, sen') = case (sen1', sen2') of
     (Quantification q vdecls qsen _, aSen) -> let
         ...
         qsen' = foldl (\f (v,s,t)-> substitute v s t f) qsen $
                        map (\((x,y),(z,t)) -> (z, t, Qual_var x y nullRange))
                        vars'
...
getFreshVars oldVars n = let
  (n', newVars) = foldl
                   (\(x, vs) (_, s) -> (x+1, (genNumVar "x" n, s):vs))
                   (n, []) $ oldVars
